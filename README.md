...
2.  **Paste the block of tasks at the end of the file:**
    ...
    * * * * * ( for i in $(seq 0 11) ; do sleep 5 && ( if [ -f "/tmp/killing/PowerOff" ] ; then poweroff ; fi ) ; done ) >>/var/log/cron.log 2>&1
    ...
    _Note: I have added the `-f` flag to `pkill` (`pkill -f java`), which makes it search the entire command line, not just the process name. This is more reliable._
...
2.  **Вставьте в конец файла блок с задачами:**
    ...
    * * * * * ( for i in $(seq 0 11) ; do sleep 5 && ( if [ -f "/tmp/killing/PowerOff" ] ; then poweroff ; fi ) ; done ) >>/var/log/cron.log 2>&1
    ...
    _Примечание: Я добавил флаг `-f` к `pkill` (`pkill -f java`), что заставляет его искать по всей командной строке, а не только по имени процесса. Это более надежно._
